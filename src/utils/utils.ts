...
export const convertXYtoViewPort = (x: number, y: number, rootId?: string) => {
    let rec = document.getElementById(rootId || 'viewport') as SVGGElement | null;
    if(!rec) return;
...
(window as any).convertXYtoViewPort = convertXYtoViewPort;
